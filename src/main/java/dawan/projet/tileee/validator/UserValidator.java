...
//	}
	...
	private static Email configureIMAP( Email email, Object subject, Object htmlContent, String to ) throws Exception {
		...
		email.setAuthenticator(new DefaultAuthenticator("tileee.dawan@gmail.com", "dawan2019"));
		email.setSSLOnConnect(true);
		email.setFrom( "Tileee <tileee.dawan@gmail.com>" );
	...
	public static void sendEmail(String from, String to, String subject, String htmlContent, String attachmentUrl,
		...
		try {
			...
			if (!attachmentUrl.equals("")) {
				...
				EmailAttachment attachement = new EmailAttachment(); // Objet Java pièce jointe
				...
				email.addHeader("Content-Type", "text/html; charset=UTF-8"); // Pour que les balises HTML soient exécutées mais pas affichées
				attachement.setName(name); // le nom du fichier quand le destinataire enregistre la pièce
				email.attach(attachement); // attacher la pièce à l'email
				email.send();
			} else {
				// Sans pièce jointe
				...
				email.addHeader("Content-Type", "text/html; charset=UTF-8"); // Pour que les balises soient exécutées mais pas affichées
				email.setFrom("Tileee <tileee.dawan@gmail.com>");
				email.send();
			}
